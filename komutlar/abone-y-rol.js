...
exports.run = async (client, message) => {
  ...
  if (!rol)
    return message.channel.send(
      `<:no:827259669574582342> **Bir rol etiketlemen gerekmekte örnek: __${ayarlar.prefix}abone-yetkili-rol @rol__**`
    );

  database.set(`aboneyetkilisi.${message.guild.id}`, rol.id);
  message.channel.send(
    `<:yes:827259552298303559> **Abone yetkilisi başarıyla "${rol}" olarak ayarlandı.**`
  );
};
